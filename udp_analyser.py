...
class Udp_analyser:
    ...
    def flow_record_process(self, record, time_stamp, ip):
        ...
        pkt['data_len'] = len(udp.data)
        pkt['ip_content'] = hexlify(ip.__bytes__()).decode()#why decode the content of ip? the function of ip_udp_map has encode the ip.
        record['pkt_info'].append(pkt)

    def ip_udp_map(self, record):
        ...
        for pkt in record['pkt_info']:
            ...
            # udp_header_map,because of there is no information in udp_header,
            # so the variation of the header information is 0
            pkt_map['th_seq'] = PADDING_INT
            pkt_map['4layer_hdr_len'] = len(udp) - len(udp.data)
            ...
            if DATA_LEN != 0:
                pkt_map['data'] = hexlify(udp.data[:DATA_LEN]).decode()
            else:
                pkt_map['data'] = None
